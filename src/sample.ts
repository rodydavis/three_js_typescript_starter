import { scene } from "./utils/xr-render";

import { controller1 } from "./utils/controllers/controllers";

import { gamepad } from "./utils/controllers/gamepad";
...
gamepad.addEventListener("gamepadInteraction", function (event: CustomEvent) {
  writeText(`${event.detail.type} ${event.detail.value}`);
});

(async function () {
  // Forest from Google Poly, https://poly.google.com/view/2_fv3tn3NG_
  const { scene: gltfScene } = await new Promise((resolve) =>
